 ...
 * Licencia: GNU GPL V3
 *
 * La clase SistemaConcesionario representa el sistema de gestión de un
 * concesionario de vehículos.
 * Permite la administración de vehículos, clientes, empleados, transacciones y
 * reportes.
 */
public class SistemaConcesionario {
    ...
    public boolean verificarEmpleadoCuenta(String nombreUsuario, String contraseña) {
        for (Empleado empleado : empleados) {
            if (empleado.getNombreDeUsuario().equals(nombreUsuario) && empleado.getContraseña().equals(contraseña)) {
                return true;
            }
        }
        return false;
    ...
    public LinkedList<Vehiculo> getVehiculosCliente(Cliente cliente) {
        ...
        for (int i = 0; i < cliente.getVehiculosPertenecientes().size(); i++) {
            lista.add(cliente.getVehiculosPertenecientes().get(i));
        }
        return lista;
    }
